...
void writeSPIOctets(byte regAddr, byte* octetValues, int numOctets) {
        
    // Create the message to send over SPI
    byte txBuf[numOctets + 1];
    txBuf[0] = regAddr | 0x80;
    for (int i = 0; i < numOctets; i++) {
        txBuf[i + 1] = octetValues[i];
    }
    // Send the message over SPI
    digitalWrite(ssPin, LOW);
    SPI.transfer(&txBuf[0], numOctets + 1);
    digitalWrite(ssPin, HIGH);
}
...
void tagPulse(double* accelData, double* gyroData, double clkOffset) {
    byte sendData[sizeof(accelData) + sizeof(gyroData) + sizeof(clkOffset)];
    memcpy(sendData, gyroData, sizeof(gyroData));
    memcpy(sendData + sizeof(gyroData), accelData, sizeof(accelData));
    digitalWrite(ssPin, LOW);
    SPI.transfer(0x09);
...
void sendPulse(int anchor) {
    ...
    writeSPIOctets(); // needs work, tx_time is read only?
...
unsigned long long getRxTimestamp(int anchor) {
    // Get timestamp value
    byte rxTime[5];
    unsigned long long rxTime = readRegister(RX_TIME);
...
//double clock_offset(double Distance) do we want change or just to assign it new value?
...
void calculatePosition(double& x, double& y, double& z, double* anchorA, double* anchorB, double* anchorC, double* anchorD,
    double tdoaAB, double tdoaAC, double tdoaAD, double tdoaBC, double tdoaBD, double tdoaCD) {
    // Calculate tag position based on TDOA ranging result
    double a[3] = {anchorPositions[ANCHOR_A][0], anchorPositions[ANCHOR_A][1], anchorPositions[ANCHOR_A][2]};
    double b[3] = {anchorPositions[ANCHOR_B][0], anchorPositions[ANCHOR_B][1], anchorPositions[ANCHOR_B][2]};
    double c[3] = {anchorPositions[ANCHOR_C][0], anchorPositions[ANCHOR_C][1], anchorPositions[ANCHOR_C][2]};
    double d[3] = {anchorPositions[ANCHOR_D][0], anchorPositions[ANCHOR_D][1], anchorPositions[ANCHOR_D][2]};
